...
INPUT_CSV = "hyaku.csv"
OUTPUT_JSONL = "hyaku_sarashina_oss20b.jsonl"
...
def call_local_oss(user_prompt: str) -> str:
    """Ollama(gpt-oss:20b) に投げてテキスト生成。"""
    full_prompt = SARASHINA_INSTRUCTIONS.strip() + "\n\n" + user_prompt

    payload = {
        "model": LOCAL_MODEL_NAME,
        "prompt": full_prompt,
        "stream": False,
        # "options": {"temperature": 0.4} なども追加可
    }

    r = requests.post(OLLAMA_ENDPOINT, json=payload, timeout=600)
    r.raise_for_status()
    data = r.json()
...
def main():
    print(f"[OSS20B] model = {LOCAL_MODEL_NAME}")
    ...
    with open(INPUT_CSV, newline="", encoding="utf-8") as f_in, \
        ...
        for row in reader:
            ...
            print(f"[OSS20B] No.{no} {author} ...", flush=True)

            try:
                output_text = call_local_oss(user_prompt)
            except Exception as e:
                print(f"[ERROR OSS20B] No.{no}: {e}")
                output_text = ""
