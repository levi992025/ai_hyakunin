...
INPUT_CSV = "hyaku.csv"
OUTPUT_JSONL = "hyaku_sarashina_gemma.jsonl"
...
def call_local_gemma(user_prompt: str) -> str:
    """Ollama(gemma3:4b) に投げてテキスト生成。"""
...
def main():
    print(f"[GEMMA] model = {LOCAL_MODEL_NAME}")
    ...
    with open(INPUT_CSV, newline="", encoding="utf-8") as f_in, \
        ...
        for row in reader:
            ...
            print(f"[GEMMA] No.{no} {author} ...", flush=True)

            try:
                output_text = call_local_gemma(user_prompt)
            except Exception as e:
                print(f"[ERROR GEMMA] No.{no}: {e}")
                output_text = ""
